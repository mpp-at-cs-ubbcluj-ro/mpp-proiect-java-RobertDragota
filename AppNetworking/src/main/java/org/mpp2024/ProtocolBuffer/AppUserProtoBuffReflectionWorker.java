...
public class AppUserProtoBuffReflectionWorker implements Runnable, AppObserverInterface {
    ...
    private Response handleGET_ALL_TRIP_BY_DESTINATION_FROM_TO(Request request) throws AppException {
        ...
        try {
            Iterable<Trip> list = server.Get_All_Trip_By_Destination_From_To(destination.getDestination(),destination.getDate(), destination.getStartHour(), destination.getFinishHour());
